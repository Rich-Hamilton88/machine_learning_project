...
function tableDisplay(ufcevents) {
  var tbody = d3.select("tbody");
  ufcevents.forEach((ufcRecord) => {
    var row = tbody.append("tr");
    Object.entries(ufcRecord).forEach(([key, value]) => {
...
button.on("click", function(event) {
  ...
  for (var i = 0; i < inputId.length; i++) {
	...
	if (field.trim() !== "") {
	  var filteredData = filteredData.filter(ufcevents =>
	    // match as case insensitive
		ufcevents[idName].toUpperCase().trim() ===
